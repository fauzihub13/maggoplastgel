...
@push('scripts')
    ...
    <script>
        ...
        var labels_month = @json($chart_orders_month['labels']); // Data tanggal
        var data_month = @json($chart_orders_month['data']);  // Data total pesanan
    
        var labels_week = @json($chart_orders_week['labels']); // Data tanggal
        var data_week = @json($chart_orders_week['data']); 

        var ctx = document.getElementById('orderChart').getContext('2d');
        var orderChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels_week, // Tanggal
                datasets: [{
                    label: 'Total penjualan',
                    data: data_week, // Total pesanan
                    borderWidth: 5,
                    borderColor: '#6777ef',
        ...
        $('#week-chart').on('click', function(){
            updateChart(labels_week, data_week);
        });

        $('#month-chart').on('click', function(){
            updateChart(labels_month, data_month);
        });
    </script>
@endpush
